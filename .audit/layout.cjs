...
const results=[];
for(const size of [{width:390,height:844},{width:1366,height:768},{width:390,height:500}]){
...
const small=[...body.querySelectorAll('input,button')].filter(e=>{const r=e.getBoundingClientRect();return r.width&&r.height&&(r.width<36||r.height<36)});if(small.length)totals.smallTargets++;
...
if(result.bad.length){const x=result.bad[0];await p.evaluate(x=>{currentQ=(x.bank==='normal'?BANKS:ADV)[x.code][x.index-1]();subs=[];paint();document.getElementById('quiz-screen').scrollTop=0},x);await p.waitForTimeout(100);await p.screenshot({path:'.audit/clipped-'+size.width+'-'+size.height+'.png'});}
}
await p.setViewportSize({width:390,height:844});await p.evaluate(()=>startQuiz('b1'));await p.waitForTimeout(100);await p.screenshot({path:'.audit/quiz-mobile.png'});
await p.evaluate(()=>goMap());await p.waitForTimeout(100);await p.screenshot({path:'.audit/map-mobile.png'});
await p.setViewportSize({width:1366,height:768});await p.evaluate(()=>goMap());await p.waitForTimeout(100);await p.screenshot({path:'.audit/map-desktop.png'});
fs.writeFileSync('.audit/layout-results.json',JSON.stringify(results,null,2));await b.close();server.close();})().catch(e=>{console.error(e);process.exit(1)});
